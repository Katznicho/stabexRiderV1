 ...
 */
  const fetcher = async (): Promise<any> => {
    try {
      ...
      // Loop through the params object to create a JSON object
      //if form 
      // Conditionally pass params if it's defined
      const postData = params ? params : undefined;

      // Send the POST request with the provided data
      const { data } = await axiosInstance.post<T>(endpoint, postData);
